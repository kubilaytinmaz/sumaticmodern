...
import { ArrowLeft, MapPin, Monitor, LineChart as LineChartIcon, BarChart3 } from 'lucide-react';
...
import DeviceRevenueTable from '@/components/dashboard/DeviceRevenueTable';
...
const SLOT_OPTIONS = [
  { value: '7', label: '7 Mum', slots: 7 },
  { value: '14', label: '14 Mum', slots: 14 },
  { value: '30', label: '30 Mum', slots: 30 },
] as const;
...
const CHART_TYPE_OPTIONS = [
  ...
  { value: 'area-labels', label: 'Alan + Değer Etiketleri' },
  { value: 'area-dual', label: 'Alan + Çift Eksen' },
  { value: 'area-percent', label: 'Alan + % Değişim' },
  { value: 'area-all', label: 'Alan + Tüm Özellikler' },
];
...
export default function AllDevicesPage() {
  ...
  const [isDeviceTableLoading, setIsDeviceTableLoading] = useState(true);
  const [isDeviceTableRefreshing, setIsDeviceTableRefreshing] = useState(false);
  ...
  // Fetch per-device summary for the table
  const fetchDeviceTableData = useCallback(async (selectedPeriod: string, selectedSlots: number, isInitial: boolean = false) => {
    if (isInitial) {
      setIsDeviceTableLoading(true);
    } else {
      setIsDeviceTableRefreshing(true);
    }
    const apiUrl = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8000';
    try {
      const res = await fetch(
        `${apiUrl}/api/v1/charts/devices/per-device-summary?period=${selectedPeriod}&slots=${selectedSlots}&metric=${metric}`
      );
      if (res.ok) {
        const data = await res.json();
        setDeviceTableData(data.devices || []);
      }
    } catch (error) {
      console.error('Failed to fetch device table data:', error);
    } finally {
      if (isInitial) {
        setIsDeviceTableLoading(false);
      } else {
        setIsDeviceTableRefreshing(false);
      }
    }
  }, [metric]);

  // Fetch chart data for Daily Revenue Table
  const fetchTableData = useCallback(async (selectedPeriod: string, selectedSlots: number, isInitial: boolean = false) => {
    if (isInitial) {
    ...
    try {
      const res = await fetch(
        `${apiUrl}/api/v1/charts/devices/all-chart-data?period=${selectedPeriod}&slots=${selectedSlots}&metric=${metric}`
      );
      if (res.ok) {
        const data = await res.json();
        setTableData(data);
  ...
  // Fetch chart data for all devices
  // isInitial: true ise tam ekran loading, false ise arka planda yenileme (kartlar korunur)
  const fetchChartData = useCallback(async (isInitial: boolean = false) => {
    if (isInitial) {
    ...
    try {
      const res = await fetch(
        `${apiUrl}/api/v1/charts/devices/all-chart-data?period=${period}&slots=${selectedSlots}&metric=${metric}`
      );
      if (res.ok) {
        const data = await res.json();
        setChartData(data);
      }
      // Also fetch device table data
      await fetchDeviceTableData(period, selectedSlots, isInitial);
    } catch (error) {
  ...
  }, [period, metric, slotCount, fetchDeviceTableData]);

  // İlk yükleme ref'i: sadece ilk kez true ile çağıralım
  ...
  useEffect(() => {
    if (devices.length > 0) {
      const selectedSlots = SLOT_OPTIONS.find(s => s.value === slotCount)?.slots || 30;
      // tablePeriod değişimi: tableData zaten varsa background refresh, yoksa initial
      const isInitial = tableData === null;
      fetchTableData(tablePeriod, selectedSlots, isInitial);
    }
  }, [tablePeriod, devices.length, fetchTableData, slotCount]);

  // Current labels
  const currentPeriodLabel = PERIOD_OPTIONS.find(p => p.value === period)?.label || 'Günlük';
  const currentSlotLabel = SLOT_OPTIONS.find(s => s.value === slotCount)?.label || '30 Mum';
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-bold tracking-tight">Tüm Cihazlar</h1>
      ...
      <Card>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            {/* "Tüm Cihazlar" Button - Selected */}
            <button
              className="p-3 rounded-lg border-2 transition-all min-w-[120px] shadow-md border-primary bg-primary/10 ring-2 ring-primary/20"
            >
            ...
            {devices.map((device) => (
              <button
                ...
                className="p-3 rounded-lg border-2 transition-all min-w-[120px] hover:shadow-md border-border hover:border-primary/50 bg-background"
              >
                <div className="text-center">
      ...
      {/* Mum Sayısı & Periyot & Metrik Selectors */}
      <div className="flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          <label className="text-sm font-medium">Mum Sayısı:</label>
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <CardTitle className="text-base">
                  Toplam Ciro — Tüm Cihazlar {metric !== 'total' ? `— ${currentMetricLabel}` : ''}
                </CardTitle>
                <CardDescription>{currentSlotLabel} · {currentPeriodLabel.toLowerCase()} periyot · Toplam kümülatif ciro: {formatMoney(totalAllDevices)}</CardDescription>
              </div>
            </div>
        ...
        <CardContent>
          ...
          {!isInitialLoading && (
            <ChartMetricsHeader
              type="revenue"
              metrics={{
                totalRevenue: chartData?.monthly_revenue || 0,
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div>
                <CardTitle className="text-base">
                  Periyodik Artış — Toplam
                </CardTitle>
                <CardDescription>
                  {currentSlotLabel} · {currentPeriodLabel.toLowerCase()} periyot · Sadece artış değerleri
                </CardDescription>
              </div>
            </div>
      ...
      {/* Device Revenue Table — Cihaz Bazlı Ciro Tablosu */}
      <Card>
        <CardHeader>
          <CardTitle>Cihaz Bazlı Ciro Tablosu</CardTitle>
          <CardDescription>
            {currentPeriodLabel.toLowerCase()} periyodunda her cihazın güncel değeri, son artışı ve offline süresi
          </CardDescription>
        </CardHeader>
        <CardContent>
          <DeviceRevenueTable
            data={deviceTableData}
            isLoading={isDeviceTableLoading || isInitialLoading}
            period={period}
            onDeviceClick={(id) => router.push(`/devices/${id}`)}
          />
        </CardContent>
      </Card>
    </div>
